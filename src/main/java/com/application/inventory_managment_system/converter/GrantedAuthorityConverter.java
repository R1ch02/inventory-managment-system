package com.application.inventory_managment_system.converter;
    ...
    public Collection<GrantedAuthority> convert(Jwt jwt) {
      ...
      if (resourceAccess != null && !resourceAccess.isEmpty()) {
        ...
        resourceAccess.forEach((resource, resourceClaims) -> {
          ...
          /*
           * Изменяем префикс, так как роли от клиентов находятся в разделе "resource_access" внутри названия клиента.
           * Роли привязываются к клиенту, для удобства префиксом в ковертере сделаем "ROLE_realm_"
           */
